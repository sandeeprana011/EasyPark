...
public final class Detector {
    ...
    private static final class Point {
        ...
        public ResultPoint toResultPoint() {
            return new ResultPoint((float) this.f3x, (float) this.f4y);
        }
    ...
    private ResultPoint[] getMatrixCornerPoints(Point[] bullEyeCornerPoints) throws NotFoundException {
        ...
        int dx = bullEyeCornerPoints[0].f3x - bullEyeCornerPoints[2].f3x;
        dx += dx > 0 ? 1 : -1;
        int dy = bullEyeCornerPoints[0].f4y - bullEyeCornerPoints[2].f4y;
        dy += dy > 0 ? 1 : -1;
        int targetcx = MathUtils.round(((float) bullEyeCornerPoints[2].f3x) - (((float) dx) * ratio));
        int targetcy = MathUtils.round(((float) bullEyeCornerPoints[2].f4y) - (((float) dy) * ratio));
        int targetax = MathUtils.round(((float) bullEyeCornerPoints[0].f3x) + (((float) dx) * ratio));
        int targetay = MathUtils.round(((float) bullEyeCornerPoints[0].f4y) + (((float) dy) * ratio));
        dx = bullEyeCornerPoints[1].f3x - bullEyeCornerPoints[3].f3x;
        dx += dx > 0 ? 1 : -1;
        dy = bullEyeCornerPoints[1].f4y - bullEyeCornerPoints[3].f4y;
        dy += dy > 0 ? 1 : -1;
        int targetdx = MathUtils.round(((float) bullEyeCornerPoints[3].f3x) - (((float) dx) * ratio));
        int targetdy = MathUtils.round(((float) bullEyeCornerPoints[3].f4y) - (((float) dy) * ratio));
        int targetbx = MathUtils.round(((float) bullEyeCornerPoints[1].f3x) + (((float) dx) * ratio));
        int targetby = MathUtils.round(((float) bullEyeCornerPoints[1].f4y) + (((float) dy) * ratio));
    ...
    private Point[] getBullEyeCornerPoints(Point pCenter) throws NotFoundException {
        ...
        int dx = pina.f3x - pinc.f3x;
        int dy = pina.f4y - pinc.f4y;
        int targetcx = MathUtils.round(((float) pinc.f3x) - (((float) dx) * ratio));
        int targetcy = MathUtils.round(((float) pinc.f4y) - (((float) dy) * ratio));
        int targetax = MathUtils.round(((float) pina.f3x) + (((float) dx) * ratio));
        int targetay = MathUtils.round(((float) pina.f4y) + (((float) dy) * ratio));
        dx = pinb.f3x - pind.f3x;
        dy = pinb.f4y - pind.f4y;
        int targetdx = MathUtils.round(((float) pind.f3x) - (((float) dx) * ratio));
        int targetdy = MathUtils.round(((float) pind.f4y) - (((float) dy) * ratio));
        int targetbx = MathUtils.round(((float) pinb.f3x) + (((float) dx) * ratio));
        int targetby = MathUtils.round(((float) pinb.f4y) + (((float) dy) * ratio));
        if (isValid(targetax, targetay) && isValid(targetbx, targetby) && isValid(targetcx, targetcy) && isValid(targetdx, targetdy)) {
    ...
    private boolean[] sampleLine(Point p1, Point p2, int size) {
        ...
        float dx = (((float) (p2.f3x - p1.f3x)) * moduleSize) / d;
        float dy = (((float) (p2.f4y - p1.f4y)) * moduleSize) / d;
        float px = (float) p1.f3x;
        float py = (float) p1.f4y;
    ...
    private boolean isWhiteOrBlackRectangle(Point p1, Point p2, Point p3, Point p4) {
        Point p12 = new Point(p1.f4y + 3, null);
        Point p22 = new Point(p2.f4y - 3, null);
        Point p32 = new Point(p3.f4y - 3, null);
        Point p42 = new Point(p4.f4y + 3, null);
    ...
    private int getColor(Point p1, Point p2) {
        float d = distance(p1, p2);
        float dx = ((float) (p2.f3x - p1.f3x)) / d;
        float dy = ((float) (p2.f4y - p1.f4y)) / d;
        int error = 0;
        float px = (float) p1.f3x;
        float py = (float) p1.f4y;
        boolean colorModel = this.image.get(p1.f3x, p1.f4y);
    ...
    private static float distance(Point a, Point b) {
        return MathUtils.distance(a.f3x, a.f4y, b.f3x, b.f4y);
    }
}
