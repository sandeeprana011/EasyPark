...
public final class ITFReader extends OneDReader {
    ...
    private static final int f12N = 1;
    static final int[][] PATTERNS;
    private static final int[] START_PATTERN;
    private static final int f13W = 3;
    ...
    static {
        ...
        START_PATTERN = new int[]{f12N, f12N, f12N, f12N};
        END_PATTERN_REVERSED = new int[]{f12N, f12N, f13W};
        PATTERNS = new int[][]{new int[]{f12N, f12N, f13W, f13W, f12N}, new int[]{f13W, f12N, f12N, f12N, f13W}, new int[]{f12N, f13W, f12N, f12N, f13W}, new int[]{f13W, f13W, f12N, f12N, f12N}, new int[]{f12N, f12N, f13W, f12N, f13W}, new int[]{f13W, f12N, f13W, f12N, f12N}, new int[]{f12N, f13W, f13W, f12N, f12N}, new int[]{f12N, f12N, f12N, f13W, f13W}, new int[]{f13W, f12N, f12N, f13W, f12N}, new int[]{f12N, f13W, f12N, f13W, f12N}};
    }
    ...
    public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> hints) throws FormatException, NotFoundException {
        ...
        decodeMiddle(row, startRange[f12N], endRange[0], result);
        ...
        if (lengthOK) {
            ...
            resultPointArr[0] = new ResultPoint((float) startRange[f12N], (float) rowNumber);
            resultPointArr[f12N] = new ResultPoint((float) endRange[0], (float) rowNumber);
    ...
    private static void decodeMiddle(BitArray row, int payloadStart, int payloadEnd, StringBuilder resultString) throws NotFoundException {
        ...
        while (payloadStart < payloadEnd) {
            ...
            for (int k = 0; k < 5; k += f12N) {
                int twoK = k << f12N;
                counterBlack[k] = counterDigitPair[twoK];
                counterWhite[k] = counterDigitPair[twoK + f12N];
            }
            ...
            for (int i$ = 0; i$ < arr$.length; i$ += f12N) {
                payloadStart += arr$[i$];
            }
        }
    }
    ...
    int[] decodeStart(BitArray row) throws NotFoundException {
        ...
        this.narrowLineWidth = (startPattern[f12N] - startPattern[0]) >> 2;
    ...
    int[] decodeEnd(BitArray row) throws NotFoundException {
        ...
        try {
            ...
            endPattern[0] = row.getSize() - endPattern[f12N];
            endPattern[f12N] = row.getSize() - temp;
            return endPattern;
        } finally {
            row.reverse();
        }
    ...
    private static int[] findGuardPattern(BitArray row, int rowOffset, int[] pattern) throws NotFoundException {
        ...
        for (int x = rowOffset; x < width; x += f12N) {
            if ((row.get(x) ^ isWhite) != 0) {
                counters[counterPosition] = counters[counterPosition] + f12N;
            } else {
                if (counterPosition != patternLength - 1) {
                    counterPosition += f12N;
                } else if (OneDReader.patternMatchVariance(counters, pattern, MAX_INDIVIDUAL_VARIANCE) < MAX_AVG_VARIANCE) {
                    return new int[]{patternStart, x};
                } else {
                    patternStart += counters[0] + counters[f12N];
                    System.arraycopy(counters, 2, counters, 0, patternLength - 2);
                    counters[patternLength - 2] = 0;
                    counters[patternLength - 1] = 0;
                    counterPosition--;
                }
                counters[counterPosition] = f12N;
                if (isWhite) {
                    isWhite = false;
                } else {
                    isWhite = true;
